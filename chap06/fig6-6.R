...
plot.tern <- function(d, file) {
  ...
  p <- p + theme_anticlockwise() + theme_rotate(45)
  p <- p + Tlab('', labelarrow = '$\\theta_2')
  p <- p + Rlab('', labelarrow = '$\\theta_1')
  p <- p + Llab('', labelarrow = '$\\theta_3')
  p <- p + scale_T_continuous(breaks = lab_seq, labels = lab_seq)
  p <- p + scale_R_continuous(breaks = lab_seq, labels = lab_seq)
  p <- p + scale_L_continuous(breaks = lab_seq, labels = lab_seq)
